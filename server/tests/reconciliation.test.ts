...
describe('reconciliation interface', async () => {
  it('shows proposed debit and credit entries in the preview', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    const paymentId = await recordPayment(db, sellerId, 40000, 'REF-PREVIEW');

    const preview = (await proposeLedgerUpdate(db, AGENT, {
  ...
  it('shows the affected invoice and its balance change', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    const paymentId = await recordPayment(db, sellerId, 40000, 'REF-BALCHANGE');
  ...
  it('shows the supporting source records', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    const paymentId = await recordPayment(db, sellerId, 40000, 'REF-SUPPORT');
  ...
  it('reports a clean reconciliation for a consistent ledger', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    const paymentId = await recordPayment(db, sellerId, 60000, 'REF-CLEAN');
    await allocatePayment(db, sellerId, paymentId, invoiceId, 60000);
  ...
  it('links each allocation back to its journal entry', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    const paymentId = await recordPayment(db, sellerId, 60000, 'REF-TRACE');
    const entryId = await allocatePayment(db, sellerId, paymentId, invoiceId, 60000);

    const { rows } = await reconcileSeller(db, sellerId);
  ...
  it('detects drift when the cached balance is tampered with', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    const paymentId = await recordPayment(db, sellerId, 60000, 'REF-DRIFT');
    await allocatePayment(db, sellerId, paymentId, invoiceId, 60000);

    // Simulate a code path that moved the cached balance without going
    // through the allocation ledger.
    await db.run(`UPDATE invoices SET balance_cents = 12345 WHERE id = ?`, [invoiceId]);
  ...
  it('detects drift in a payment\'s cached unallocated amount', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    const paymentId = await recordPayment(db, sellerId, 60000, 'REF-PDRIFT');
    await allocatePayment(db, sellerId, paymentId, invoiceId, 60000);

    await db.run(`UPDATE payments SET unallocated_cents = 999 WHERE id = ?`, [paymentId]);
  ...
  it('flags an over-settled invoice rather than clamping it', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    // Two payments each covering the full invoice, both allocated.
    const p1 = await recordPayment(db, sellerId, 100000, 'REF-OVER-A');
    const p2 = await recordPayment(db, sellerId, 100000, 'REF-OVER-B');
    await allocatePayment(db, sellerId, p1, invoiceId, 100000);
    ...
    await db.run(
      `INSERT INTO payment_allocations
       ...
       VALUES ('alloc_forced', ?, ?, ?, 50000, 'active')`, [sellerId, p2, invoiceId]);

    const { summary } = await reconcileSeller(db, sellerId);
    expect(
      summary.drift.some((d) => d.kind === 'oversettled_invoice'),
    ).toBe(true);
    expect((await deriveInvoiceState(db, invoiceId)).balance_cents).toBe(-50000);
  });

  it('shows reminder state alongside each invoice', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    const paymentId = await recordPayment(db, sellerId, 100000, 'REF-REM');
  ...
  it('reports account balances that net to zero across the ledger', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    const paymentId = await recordPayment(db, sellerId, 60000, 'REF-ACCT');
    await allocatePayment(db, sellerId, paymentId, invoiceId, 60000);
  ...
  it('excludes reversed entries from account balances', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    const entryId = await issueInvoice(db, sellerId, invoiceId);

    const before = (await accountBalances(db, sellerId)).find((b) => b.code === '1100')!;
    expect(before.net_cents).toBe(100000);

    await reverseLedgerEntry(db, APPROVER, entryId, 'issued in error');

    const after = (await accountBalances(db, sellerId)).find((b) => b.code === '1100')!;
    expect(after.net_cents).toBe(0);
  });

  it('surfaces unapplied cash separately from outstanding AR', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    await recordPayment(db, sellerId, 60000, 'REF-UNAPPLIED');
  ...
  it('stays consistent after a full issue-pay-allocate cycle', async () => {
    const { db, sellerId, invoiceId, otherInvoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    await issueInvoice(db, sellerId, otherInvoiceId);
    const p1 = await recordPayment(db, sellerId, 100000, 'REF-CYCLE-A');
    await allocatePayment(db, sellerId, p1, invoiceId, 100000);
  ...
  it('reports the authoritative system on the summary', async () => {
    const { db, sellerId } = await makeTestDb({ authoritativeSystem: 'external' });
    const { summary } = await reconcileSeller(db, sellerId);
    expect(summary.authoritative_system).toBe('external');
  });

  it('records the expected state needed to revalidate at post time', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
    const paymentId = await recordPayment(db, sellerId, 40000, 'REF-EXPECTED');
  ...
  it('keeps the reconciliation readable after a credit note', async () => {
    const { db, sellerId, invoiceId } = await makeTestDb();
    await issueInvoice(db, sellerId, invoiceId);
